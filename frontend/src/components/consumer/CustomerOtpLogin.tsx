 ...
 *
 * OTP ownership/delivery is entirely a backend concern (see
 * backend/notifications.py's module docstring) — customer OTP is
 * generated and verified locally by Lokl's own backend, same as
 * merchant/rider login OTP, regardless of which notification provider is
 * active. This component previously integrated MSG91's OTP Widget
 * directly (Custom UI / exposeMethods mode) for a period, which moved
 * verification client-side and required a second backend call to confirm
 * an access-token — that was reverted because the widget's message body
 * was MSG91's own fixed template and couldn't be reworded to Lokl's own
