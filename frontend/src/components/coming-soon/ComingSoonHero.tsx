...
/**
 * ComingSoonHero — G15. Same visual architecture as the real HeroCarousel
 * (frontend/src/components/consumer/HeroCarousel.tsx) — padded rounded card,
 * cream gradient scrim, font-display headline with an orange-highlighted
 * phrase — but HeroCarousel itself isn't reusable here: it fetches
 * GET /api/feed/delivery-status and renders a live LIVE/AWAY ETA badge,
 * exactly the "fake ETA on a non-transactional page" this page must not
 * show. This is a single static slide (no carousel/autoplay machinery
 * needed) with a static "Launching soon" pill instead.
 *
        ...
        <div className="relative rounded-2xl overflow-hidden bg-[#0A1F5C] min-h-[380px] md:min-h-[420px]">
          ...
          <div className="relative flex flex-col max-w-2xl px-5 md:px-10 lg:px-12 pt-6 md:pt-10 pb-16 md:pb-20 min-h-[380px] md:min-h-[420px]">
            ...
            <p className="mt-2.5 md:mt-3 text-[13px] md:text-base text-[#0A1F5C]/75 md:text-[#475569] max-w-md leading-relaxed">
              Shop from the stores around you, discover local brands and get what you need without leaving your neighbourhood.
            </p>
          ...
          <div className="absolute bottom-5 left-5 md:bottom-6 md:left-10 lg:left-12">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-md px-3 py-1.5 shadow-[0_2px_8px_rgba(10,31,92,0.12)]">
              <Sparkles size={13} className="text-[#E68910]" />
              <span className="text-[11px] font-bold text-[#0A1F5C]">Launching soon &middot; Bhilai</span>
            </div>
          </div>
        </div>
      </div>
