...
/** "nehru-nagar" -> "Nehru Nagar" — good enough for a heading; the "Shop by
 *  Area" tiles are the primary entry point and already show the real name,
 *  this is just the /stores?area= deep-link's own display fallback. */
function prettifyAreaSlug(slug: string): string {
...
function StoresInner() {
  ...
  useEffect(() => {
    setStores(null);
    const params: { limit: number; lat?: number; lng?: number; area?: string } = { limit: 60 };
  ...
  }, [lat, lng, hasLocation, area]);

  const loading = stores === null;
  const list = stores ?? [];
  const areaLabel = area ? prettifyAreaSlug(area) : "";
    ...
    <div className="flex-1 flex flex-col bg-[#FDFBF7]">
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-8 pt-10">
        ...
        <p className="text-[#595959] mt-2">
          {loading
            ? "Loading…"
            : `${list.length} trusted local store${list.length !== 1 ? "s" : ""}${!area && hasLocation ? " · sorted by distance" : ""}`}
        </p>
        {loading ? (
...
export default function StoresPage() {
  return (
    <Suspense fallback={<div className="flex-1 bg-[#FDFBF7]" />}>
      <StoresInner />
    </Suspense>
  );
}
