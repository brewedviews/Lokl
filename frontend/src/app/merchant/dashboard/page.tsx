...
interface DashboardData {
  revenue: number; orders: number; repeat_rate: number; conversion: number;
...
export default function MerchantDashboardPage() {
  ...
  useEffect(() => {
    apiClient.get<DashboardData>("/api/merchant/dashboard").then((r) => setStats(r.data)).catch(() => {});
  }, []);
        ...
        <>
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={k.label} className="bg-white border border-[#E5E2DC] rounded-2xl p-5">
                ...
                <div className="font-display text-3xl font-bold text-[#1A2B4C] mt-3">{k.value}</div>
          ...
          <div className="mt-8 grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white border border-[#E5E2DC] rounded-2xl p-6">
              <h3 className="font-display text-xl font-bold text-[#1A2B4C] mb-5">Revenue this week</h3>
              <div className="flex items-end gap-2 h-48">
                {stats.trends.map((d) => {
                  const max = Math.max(...stats.trends.map((x) => x.revenue));
                  const h = (d.revenue / max) * 100;
                  // Dynamic width — must be inline style.
                  return (
            ...
            <div className="bg-white border border-[#E5E2DC] rounded-2xl p-6">
              <h3 className="font-display text-xl font-bold text-[#1A2B4C] mb-4">Top products</h3>
