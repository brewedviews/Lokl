...
/**
 * MerchantLayoutClient — sidebar nav + per-route auth/onboarding-state guard.
 *
 * Split out of app/merchant/layout.tsx (which is now a server component so
 * it can export real `metadata` — a "use client" layout segment can't).
 * layout.tsx renders this unchanged, just wrapped for metadata purposes.
 *
 * Guards:
...
const PUBLIC = ["/merchant/login", "/merchant/register", "/merchant/terms", "/"];
...
export function MerchantLayoutClient({ children }: { children: React.ReactNode }) {
